...
@class LingYeNativeExpressBannerAd;

NS_ASSUME_NONNULL_BEGIN

@protocol LingYeNativeExpressBannerAdDelegate <NSObject>
@optional

///  广告加载成功
///  在此方法中调用 showAdFromView:frame 方法
- (void)lingye_nativeExpressBannerAdDidLoad:(LingYeNativeExpressBannerAd *)bannerAd;

/// 广告加载失败
- (void)lingye_nativeExpressBannerAd:(LingYeNativeExpressBannerAd *)bannerAd didLoadFailWithError:(NSError *_Nullable)error;

/// 广告将要曝光
- (void)lingye_nativeExpressBannerAdWillBecomVisible:(LingYeNativeExpressBannerAd *)bannerAd;

/// 广告曝光失败
- (void)lingye_nativeExpressBannerAdBecomVisibleFail:(LingYeNativeExpressBannerAd *)bannerAd error:(NSError *_Nullable)error;

/// 广告被点击
- (void)lingye_nativeExpressBannerAdDidClick:(LingYeNativeExpressBannerAd *)bannerAd;
 
/// 广告被关闭
- (void)lingye_nativeExpressBannerAdClosed:(LingYeNativeExpressBannerAd *)bannerAd;

/// 广告详情页给关闭
- (void)lingye_nativeExpressBannerAdViewDidCloseOtherController:(LingYeNativeExpressBannerAd *)bannerAd;

@end

@interface LingYeNativeExpressBannerAd : NSObject
...
/// 【必选参数】 广告的代理
@property(nonatomic,weak)id<LingYeNativeExpressBannerAdDelegate> delegate;
...
/// 初始化
/// LingYeAdSlot 广告id
- (instancetype)initWithSlot:(LingYeAdSlot *)slot size:(CGSize)size NS_DESIGNATED_INITIALIZER;
