 ...
 */
@interface CJZFImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `CJZFImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) CJZFImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `CJZFImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 * When enabling this option, we will sync back the image from weak maptable to strong cache during next time top level `cjzf_setImage` function call.
 ...
 */
@property (assign, nonatomic) CJZFImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The dispatch queue attr for ioQueue. You can config the QoS and concurrent/serial to internal IO queue. The ioQueue is used by CJZFImageCache to access read/write for disk data.
...
/**
 * The custom memory cache class. Provided class instance must conform to `CJZFMemoryCache` protocol to allow usage.
 * Defaults to built-in `CJZFMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `CJZFDiskCache` protocol to allow usage.
 * Defaults to built-in `CJZFDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
