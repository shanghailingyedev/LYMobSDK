...
typedef NSData * _Nullable (^CJZFWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol CJZFWebImageDownloaderDecryptor <NSObject>
...
/// @note If nil is returned, the image download will be marked as failed with error `CJZFWebImageErrorBadImageData`
...
*/
@interface CJZFWebImageDownloaderDecryptor : NSObject <CJZFWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull CJZFWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull CJZFWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface CJZFWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) CJZFWebImageDownloaderDecryptor *base64Decryptor;
