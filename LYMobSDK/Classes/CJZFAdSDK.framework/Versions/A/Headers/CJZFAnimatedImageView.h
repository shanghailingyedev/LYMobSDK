...
#import "CJZFAnimatedImage.h"
#import "CJZFAnimatedImagePlayer.h"

/**
 A drop-in replacement for UIImageView/NSImageView, you can use this for animated image rendering.
 Call `setImage:` with `UIImage(NSImage)` which conforms to `CJZFAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface CJZFAnimatedImageView : UIImageView
/**
 ...
 @warning Pay attention if you directly update the player's property like `totalFrameCount`, `totalLoopCount`, the same property on `CJZFAnimatedImageView` may not get synced.
 */
@property (nonatomic, strong, readonly, nullable) CJZFAnimatedImagePlayer *player;
...
/// Asynchronous setup animation playback mode. Default mode is CJZFAnimatedImagePlaybackModeNormal.
@property (nonatomic, assign) CJZFAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `CJZFAnimatedImage` with animated coder which conform to `CJZFProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `CJZFAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
