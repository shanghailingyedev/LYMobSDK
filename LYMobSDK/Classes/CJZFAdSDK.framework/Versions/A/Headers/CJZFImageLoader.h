...
typedef void(^CJZFImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^CJZFImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `CJZFWebImageManager` when you specify `CJZFWebImageRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `CJZFWebImageErrorCacheNotModified` error. (UIImage)
 @note If you don't implement `CJZFWebImageRefreshCached` support, you do not need to care about this context option.
 */
FOUNDATION_EXPORT CJZFWebImageContextOption _Nonnull const CJZFWebImageContextLoaderCachedImage;

#pragma mark - Helper method

/**
 This is the built-in decoding process for image download from network or local file.
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with CJZFWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable CJZFImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, CJZFWebImageOptions options, CJZFWebImageContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `CJZFWebImageProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with CJZFWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable CJZFImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<CJZFWebImageOperation> _Nonnull operation, CJZFWebImageOptions options, CJZFWebImageContext * _Nullable context);
 ...
 */
FOUNDATION_EXPORT id<CJZFProgressiveImageCoder> _Nullable CJZFImageLoaderGetProgressiveCoder(id<CJZFWebImageOperation> _Nonnull operation);
 ...
 */
FOUNDATION_EXPORT void CJZFImageLoaderSetProgressiveCoder(id<CJZFWebImageOperation> _Nonnull operation, id<CJZFProgressiveImageCoder> _Nullable progressiveCoder);

#pragma mark - CJZFImageLoader
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `CJZFImageLoaderDecodeImageData` or `CJZFImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
 ...
 @note It's your responsibility to load the image in the desired global queue(to avoid block main queue). We do not dispatch these method call in a global queue but just from the call queue (For `CJZFWebImageManager`, it typically call from the main queue).
*/
@protocol CJZFImageLoader <NSObject>
 ...
 */
- (nullable id<CJZFWebImageOperation>)requestImageWithURL:(nullable NSURL *)url
                                                options:(CJZFWebImageOptions)options
                                                context:(nullable CJZFWebImageContext *)context
                                               progress:(nullable CJZFImageLoaderProgressBlock)progressBlock
                                              completed:(nullable CJZFImageLoaderCompletedBlock)completedBlock;


/**
 Whether the error from image loader should be marked indeed un-recoverable or not.
 If this return YES, failed URL which does not using `CJZFWebImageRetryFailed` will be blocked into black list. Else not.
