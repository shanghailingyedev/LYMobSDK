...
#import "CJZFWebImageDownloader.h"
#import "CJZFWebImageOperation.h"

/**
 Describes a downloader operation. If one wants to use a custom downloader op, it needs to inherit from `NSOperation` and conform to this protocol
 For the description about these methods, see `CJZFWebImageDownloaderOperation`
 ...
 */
@protocol CJZFWebImageDownloaderOperation <NSURLSessionTaskDelegate, NSURLSessionDataDelegate>
@required
...
- (nullable id)addHandlersForProgress:(nullable CJZFWebImageDownloaderProgressBlock)progressBlock
                            completed:(nullable CJZFWebImageDownloaderCompletedBlock)completedBlock;

- (nullable id)addHandlersForProgress:(nullable CJZFWebImageDownloaderProgressBlock)progressBlock
                            completed:(nullable CJZFWebImageDownloaderCompletedBlock)completedBlock
                        decodeOptions:(nullable CJZFImageCoderOptions *)decodeOptions;
...
// These operation-level config was inherited from downloader. See `CJZFWebImageDownloaderConfig` for documentation.
...
/**
 The download operation class for CJZFWebImageDownloader.
 */
@interface CJZFWebImageDownloaderOperation : NSOperation <CJZFWebImageDownloaderOperation>
 ...
 * For example, if we config [200, 400) but server response is 503, the download will fail with error code `CJZFWebImageErrorInvalidDownloadStatusCode`.
 ...
 * For example, if we config ["image/png"] but server response is "application/json", the download will fail with error code `CJZFWebImageErrorInvalidDownloadContentType`.
 ...
 */
@property (assign, nonatomic, readonly) CJZFWebImageDownloaderOptions options;

/**
 * The context for the receiver.
 */
@property (copy, nonatomic, readonly, nullable) CJZFWebImageContext *context;

/**
 *  Initializes a `CJZFWebImageDownloaderOperation` object
 *
 *  @see CJZFWebImageDownloaderOperation
 *
...
- (nonnull instancetype)initWithRequest:(nullable NSURLRequest *)request
                              inSession:(nullable NSURLSession *)session
                                options:(CJZFWebImageDownloaderOptions)options
                                context:(nullable CJZFWebImageContext *)context NS_DESIGNATED_INITIALIZER;
