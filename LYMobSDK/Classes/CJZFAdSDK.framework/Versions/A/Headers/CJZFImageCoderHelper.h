...
typedef NS_ENUM(NSUInteger, CJZFImageCoderDecodeSolution) {
    /// automatically choose the solution based on image format, hardware, OS version. This keep balance for compatibility and performance. Default after CJZFWebImage 5.13.0
    CJZFImageCoderDecodeSolutionAutomatic,
    /// always use CoreGraphics to draw on bitmap context and trigger decode. Best compatibility. Default before CJZFWebImage 5.13.0
    CJZFImageCoderDecodeSolutionCoreGraphics,
    ...
    CJZFImageCoderDecodeSolutionUIKit
};
...
/// The downside is that force-decode may consume RAM and CPU, and may loss the `lazy` support (lazy CGImage can be purged when memory warning, and re-created if need), see more: `CJZFImageCoderDecodeUseLazyDecoding`
typedef NS_ENUM(NSUInteger, CJZFImageCJZFForceDecodePolicy) {
    ...
    CJZFImageCJZFForceDecodePolicyAutomatic,
    /// Never force decode input CGImage
    CJZFImageCJZFForceDecodePolicyNever,
    /// Always force decode input CGImage (only once)
    CJZFImageCJZFForceDecodePolicyAlways
};
 ...
 */
+ (UIImage * _Nullable)animatedImageWithFrames:(NSArray<CJZFImageFrame *> * _Nullable)frames;
 ...
 */
+ (NSArray<CJZFImageFrame *> * _Nullable)framesFromAnimatedImage:(UIImage * _Nullable)animatedImage NS_SWIFT_NAME(frames(from:));
 ...
 */
+ (CJZFImagePixelFormat)preferredPixelFormat:(BOOL)containsAlpha;
...
/**
 ...
 @note This scale does not change bits per components (which means RGB888 in, RGB888 out), supports 8/16/32(float) bpc. But the method in UIImage+CJZFTransform does not gurantee this.
...
/// Calculate the limited image size with the bytes, when using `CJZFImageCoderDecodeScaleDownLimitBytes`. This preserve aspect ratio and never scale up
/// @param imageSize The image size (in pixel or point defined by caller)
...
/**
 ...
 @return The decoded image
 */
+ (UIImage * _Nullable)decodedImageWithImage:(UIImage * _Nullable)image policy:(CJZFImageCJZFForceDecodePolicy)policy;
...
/**
 ...
 @return The decoded and probably scaled down image
 */
+ (UIImage * _Nullable)decodedAndScaledDownImageWithImage:(UIImage * _Nullable)image limitBytes:(NSUInteger)bytes policy:(CJZFImageCJZFForceDecodePolicy)policy;

/**
 Control the default force decode solution. Available solutions  in `CJZFImageCoderDecodeSolution`.
 @note Defaults to `CJZFImageCoderDecodeSolutionAutomatic`, which prefers to use UIKit for JPEG/HEIF, and fallback on CoreGraphics. If you want control on your hand, set the other solution.
 */
@property (class, readwrite) CJZFImageCoderDecodeSolution defaultDecodeSolution;
