// CJZFNewNetworkActivityIndicatorManager.h
...
/**
 `CJZFNewNetworkActivityIndicatorManager` manages the state of the network activity indicator in the status bar. When enabled, it will listen for notifications indicating that a session task has started or finished, and start or stop animating the indicator accordingly. The number of active requests is incremented and decremented much like a stack or a semaphore, and the activity indicator will animate so long as that number is greater than zero.

 You should enable the shared instance of `CJZFNewNetworkActivityIndicatorManager` when your application finishes launching. In `AppDelegate application:didFinishLaunchingWithOptions:` you can do so with the following code:

    [[CJZFNewNetworkActivityIndicatorManager sharedManager] setEnabled:YES];
...
@interface CJZFNewNetworkActivityIndicatorManager : NSObject
...
/**
 Decrements the number of active network requests. If this number becomes zero  decrementing, this will stop animating the status bar network activity indicator.
 */
